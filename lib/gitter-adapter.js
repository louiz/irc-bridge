...
Adapter.prototype.queueMessage = function(target, message) {
  this.sendQueue.push({'target': target, 'message': message});
  log("queueMessage: ", this.sendQueue);
  this.startSendQueue();
};

Adapter.prototype.startSendQueue = function() {
  if (!this.sending && this.sendQueue.length > 0) {
    var message = this.sendQueue.shift();
    this.sendMessage(message['target'], message['message']);
  }
};
